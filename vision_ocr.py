...
# === ChatGPTで予想生成（車番のみ、45点以内、文章・名前・点数表示なし）===
def generate_keirin_prediction(ocr_text):
    prompt = f"""
以下は競輪の出走表です。この情報をもとに、3連単フォーメーションを予想してください。

制約条件：
- 選手名ではなく、車番（数字1～9）だけで構成してください
- 表記形式は「1→2→3」だけ（“車”や名前は使わない）
- 出力は45点以内にしてください
- 余計な文章（1着候補、3連複、点数など）は一切不要です
- 出力は箇条書きのみでお願いします
